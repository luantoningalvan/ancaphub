...
export function fetchAllArticles(page = 2, pageSize = 2, order = "desc", filter = "", filterOn = "") {
  return (dispatch) => {
    axios.get(`${BASE_URL}?type=article&&page=${page}&&pageSize=${pageSize}&&orderBy=${order}${filter && `&&filter=${filter}&&filterOn=${filterOn}`}`)
      .then((articles) => {
